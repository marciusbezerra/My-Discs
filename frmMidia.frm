...
Private Sub cmdDelete_Click()
    ...
    Dim MSG As String
    Dim Cod As Long
    MSG = "Deseja excluir o tipo de mídia " & _
        Me.datPrimaryRS.Recordset("Código") & " " & _
        "e todas os discos que pertencem a ele ?"
    If MsgBox(MSG, vbYesNo, Caption) = vbNo Then Exit Sub
...
Private Sub Form_QueryUnload(Cancel As Integer, UnloadMode As Integer)
    If Editando Then
        MsgBox "Existem transações de dados na tela de tipos de mídias " & _
...
Function Valida() As Boolean
    ...
    If Trim(Me.txtFields(1).Text) = "" Then
        MsgBox "A descrição do tipo de mídia é requerida.", vbCritical, Caption
        Valida = False
        Me.txtFields(1).SetFocus
        Exit Function
    End If
    ...
    If Not RC.NoMatch Then
        If Me.datPrimaryRS.EditMode = dbEditInProgress Then
            If CLng(RC("Código")) <> CLng(Me.datPrimaryRS.Recordset("Código")) Then
                MsgBox "Mídia já cadastrada.", vbInformation, Caption
                Valida = False
                RC.Close
                Exit Function
            Else
