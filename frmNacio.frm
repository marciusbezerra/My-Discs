...
Private Sub Form_QueryUnload(Cancel As Integer, UnloadMode As Integer)
    If Editando Then
        MsgBox "Existem transações de dados na tela de nacionalidades " & _
...
Function Valida() As Boolean
    ...
    If Trim(Me.txtFields(1).Text) = "" Then
        MsgBox "A descrição da nacionalidade é requerida.", vbCritical, Caption
        Valida = False
        Me.txtFields(1).SetFocus
        Exit Function
    End If
    ...
    If Not RC.NoMatch Then
        If Me.datPrimaryRS.EditMode = dbEditInProgress Then
            If CLng(RC("Código")) <> CLng(Me.datPrimaryRS.Recordset("Código")) Then
                MsgBox "Nacionalidade já cadastrada.", vbInformation, Caption
                Valida = False
                RC.Close
                Exit Function
            Else
